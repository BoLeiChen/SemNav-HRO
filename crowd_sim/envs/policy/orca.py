...
from crowd_sim.envs.utils.state import ObservableState_nov, JointState
...
class ORCA(Policy):
    ...
    def predict(self, state, obstacle=None):
        ...
        ob = []
        for i, human_state in enumerate(state.human_states):
            human_state = ObservableState_nov(human_state.px, human_state.py, human_state.radius)
            ob.append(human_state)
        self.last_state = JointState(state.robot_state, ob, False)
