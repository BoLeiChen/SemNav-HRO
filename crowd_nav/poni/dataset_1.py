...
class SemanticMapDataset(Dataset):
            ...
            with h5py.File(path, 'r') as fp:
                ...
                for floor_id in floor_ids:
                    ...
                    if nuniq >= MIN_OBJECTS_THRESH + 2:
                        ...
                        w = numpy.size(maps[name],1)
                        h = numpy.size(maps[name],2)
    ...
    def __getitem__(self, idx):
        ...
        if self.cfg.masking_mode == 'spath':
            ...
            label["nav_space"] = nav_space
            label["nav_locs"] = nav_locs
        else:
            raise ValueError(f"Masking mode {self.cfg.masking_mode} is not implemented!")
        return input, label
        '''
        return name, semmap, fmm_dists, map_xyz_info, nav_space, nav_locs
