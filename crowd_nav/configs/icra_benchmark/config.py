...
class BaseEnvConfig(object):
    ...
    sim.train_val_scenario = 'from_data'
    sim.test_scenario = 'from_data'
    sim.square_width = 20
    sim.circle_radius = 4
    sim.human_num = 20
    sim.obstacle_num = 3
...
class BaseTrainConfig(object):
    ...
    train.target_update_interval = 100  #1000
    train.evaluation_interval = 200     #1000
    # the memory pool can roughly store 2K episodes, total size = episodes * 50
    train.capacity = 5000
    train.epsilon_start = 0.5
    train.epsilon_end = 0.1
    train.epsilon_decay = 300           #4000
    train.checkpoint_interval = 100     #1000
